...
export function LabCanvas({
  ...
  useEffect(() => {
    ...
    // 7. Controlled Wheel Scroll (Not global page scroll hijacking)
    let scrollVelocity = 0;
    let stackOffset = 0;

    const handleWheel = (e: WheelEvent) => {
      // Only scroll stack if mouse is inside canvas
      const rect = container.getBoundingClientRect();
      if (
        e.clientX >= rect.left &&
    ...
    // Keyboard Accessibility: Escape to deselect
    ...
    if (selectedProject) {
      ...
      cardMeshes.forEach((mesh) => {
        ...
        if (u.project.id === selectedProject.id) {
          // Focus Selected Card to front-and-center
          ...
          // Push non-selected cards back slightly
          gsap.to(mesh.position, {
            x: u.baseX * 1.4,
            y: u.baseY,
            z: u.baseZ - 1.2,
      ...
      // Reset back to normal stack state
      gsap.to(camera.position, {
        x: 0,
        y: 0,
        z: 9,
