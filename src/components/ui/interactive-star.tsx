...
export function InteractiveStar({
  ...
  return (
    <button
      ...
      className={cn(
        "cursor-pointer focus:outline-none transition-transform duration-500 ease-in-out hover:scale-110",
        active ? "rotate-[45deg]" : "rotate-0",
        className
      )}
      title="Click to rotate 45 deg"
    ...
    >
      <svg
        ...
        className={cn(
          "w-8 h-8 sm:w-9 sm:h-9 transition-colors duration-300",
          active ? "text-[#FF0004]" : "text-black"
        )}
      >
        <path
